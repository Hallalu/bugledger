# BUG REGRESSION CHECKLIST — 428 known bugs across 24 apps
...
## cross-cutting (70)
- [ ] AI-hallucinated dependency (slopsquatting) pulled into the build — An install/import references a package that sounds right but isn't the real project; it either…
...
- [ ] Request state stored in Worker module globals leaks across users — Under load, one user intermittently sees another user's data (auth context, cart, tenant id),…
...
- [ ] SSRF: allowlist outbound fetch targets (CWE-918, #1 AI-code flaw) — A server-side fetch/proxy can be pointed at internal metadata endpoints or arbitrary hosts.
- [ ] System prompt leakage (OWASP LLM07:2025) — Internal system-prompt rules, filters, permissions or hidden logic can be extracted by users a…
- [ ] Vector & embedding weaknesses in RAG (OWASP LLM08:2025) — Retrieved documents poison answers, or one tenant's embeddings leak into another's results.
...
- [ ] Background work dropped because it wasn't passed to ctx.waitUntil() — Analytics writes, audit logs, cache warms, or D1 upserts kicked off right before returning a R…
- [ ] Bulk import silently drops rows it can't parse — A user uploads 200 leads; 170 land, 30 vanish with no message. The loss is invisible until a c…
- [ ] Data export silently omits notes, activity history and timeline — Users export 'all my data' to leave or back up and get only the contact/lead rows — call notes…
- [ ] Export is partial or lossy — misses media, one item at a time, or PDF-only — Users trying to leave or back up find the export omits photos/attachments/voice notes, only ex…
- [ ] Uncaught QuotaExceededError when writing large blobs to localStorage — Saving images/base64/large JSON to localStorage throws QuotaExceededError past ~5MB; the write…
- [ ] Unsynced in-memory state wiped on refresh, tab-close, or crash — Everything the user typed since opening the app is gone after a refresh, an accidental close,…
- [ ] D1 'too many SQL variables' — 100 bound-parameter limit, not 999 — Bulk insert/update or a WHERE ... IN (?, ?, ...) over a dynamic array throws 'D1_ERROR: too ma…
...
- [ ] typeof does not guard a let/const in its temporal dead zone — A defensive `typeof x` guard still threw ReferenceError at runtime.
- [ ] White-screen ChunkLoadError after deploy from SPA index.html fallback — Users with the tab already open (or a stale service worker) get a blank page or 'ChunkLoadErro…
- [ ] Accounts exist with no delete-my-data path — A user asks to be deleted and there is no mechanism — deleting the app leaves everything on th…
- [ ] Collects personal data with no privacy policy — The product takes an email, name or phone number and there is no policy anywhere saying what h…
- [ ] Consent banner that is non-compliant or purely decorative — Non-essential cookies/trackers load before any consent is given, or the banner offers 'Accept…
- [ ] Data kept indefinitely with no retention policy — Every record ever created is still stored, including data for deleted or long-dormant accounts.
- [ ] Personal data or credentials written to logs — Emails, passwords, tokens or entire user records land in the console and in retained server lo…
- [ ] Personal data placed in a URL query string — Emails, tokens or identifiers appear in links, and therefore in server logs, browser history,…
- [ ] Sensitive data routed through third-party ad/analytics SDKs — Health, reproductive, financial or precise-location data is transmitted to advertising or anal…
- [ ] Failed save/sync silently drops the user's edit (no offline write queue) — User adds/edits an entry while offline or on a flaky connection; the network call fails and th…
- [ ] KV read-after-write returns stale value (eventual consistency) — Code writes a value to Workers KV then reads it back in the same or next request and gets the…
- [ ] Cache-overwrite race in AI/caching layer corrupts entries — Under concurrent requests, a cached AI response or computed value gets served for the WRONG in…
...
- [ ] AI-generated fact asserted to users without verification — A feature summary, competitor comparison or 'insight' presented to users contains details the…
- [ ] Manufactured scarcity or urgency in the UI — The page shows a countdown, 'only 3 left', or 'X people are viewing this' that is not derived…
- [ ] Placeholder or sample content shipped to real users — Lorem ipsum, TODO markers, 'John Doe' testimonials or 'your text here' appear in the live prod…
- [ ] Statistic published with no citable source — A percentage or research-style figure appears in marketing or in-app copy, and no one can say…
- [ ] Unverifiable superlative claim shipped in product copy — The product asserts it is the 'first', 'only' or 'best' of its kind, and the claim turns out t…
- [ ] Click handler on a div/span — not keyboard reachable — A control works with a mouse but cannot be reached or activated by keyboard or screen reader a…
- [ ] Document missing a lang attribute — Screen readers pronounce the page with the wrong language voice, making content hard or imposs…
- [ ] Drag-only interaction with no single-pointer alternative (WCAG 2.2 · 2.5.7 Dragging Movements) — Reorder / move / slider interactions that only work by dragging exclude users who cannot perfo…
- [ ] Focus indicator removed by outline:none — Keyboard users tabbing through the interface cannot see where they are — focus becomes invisib…
- [ ] Form field with no associated label — A screen reader reaches the input and announces only 'edit text' — the user cannot tell what t…
- [ ] Icon-only button or link with no accessible name — Screen readers announce 'button' or read out the URL; empty links appear on ~46% of pages and…
- [ ] Images with no alt attribute — Screen readers announce the raw filename (or nothing) instead of the image's meaning; present…
- [ ] Low-contrast text below the 4.5:1 minimum — Light grey body text on white is unreadable for low-vision users and in sunlight; appears on ~…
- [ ] PIN/OTP field blocks paste or uses a cognitive-test CAPTCHA (WCAG 2.2 · 3.3.8 Accessible Authentication) — A login/recovery step relies on a cognitive function test (transcribe/solve/recognise) or disa…
- [ ] Positive tabindex breaks the natural tab order — Tabbing jumps around the page unpredictably, skipping or reordering controls.
- [ ] Touch target smaller than 24×24 CSS px (WCAG 2.2 · 2.5.8 Target Size) — Small icon buttons — a row options dot, a drag handle, an inline ✕, a bell toggle — are hard t…
- [ ] Empty catch block swallows the error — A feature silently does nothing; there is no error in the console and no clue in any log about…
- [ ] No build/version stamp to correlate an error with a deploy — A bug report cannot be tied to a release, so it is impossible to tell whether a fix shipped or…
- [ ] No global error handler — front-end crashes are invisible — Users report a white screen or a dead button and there is no record anywhere that anything wen…
- [ ] Promise chain with no .catch — rejections vanish — An async action fails with no visible effect; the UI shows a spinner forever or silently stays…
- [ ] .only() left in a test suite silently skips everything else — CI stays green while running a single test; broken code ships because the rest of the suite ne…
- [ ] No tests anywhere in the project — Every regression is found by a user; there is no signal before deploy that the core loop still…
- [ ] Permanently skipped tests present as dead coverage — The suite reports passing while meaningful cases are disabled, overstating real coverage.
- [ ] Tests exist but no CI runs them — A regression reaches production even though a test covering it exists, because nobody ran the…
- [ ] Duplicate event listeners stack up on every re-render/re-mount — A click/submit/keypress handler fires 2x, 3x, Nx (double submits, doubled network calls, runaw…
- [ ] Worker subrequest limit hit by per-row D1/KV/fetch in a loop — A request that iterates over a list and does a DB/KV/fetch call per item suddenly throws 'Too…
- [ ] No Open Graph or Twitter card — shared links render as a bare URL — Pasting the product link into a message, Slack or social post shows a naked URL with no title,…
- [ ] No viewport meta — the page renders desktop-width on phones — On a phone the whole layout is zoomed out and tiny, or controls run off the edge of the screen.
- [ ] Page has no <title> or meta description — Browser tabs read 'index.html' or the domain, and search results show a machine-generated snip…
- [ ] Later same-specificity CSS rule silently overrides mobile media query — A mobile-only sizing rule had no effect; the element overflowed off-screen on phones.
...
- [ ] Cross-account data leak: shared localStorage cache not cleared on sign-out — On a shared device, the next person to sign in loaded the previous user's leads and invoices f…
- [ ] Owner email exposed client-side in admin.js — The admin owner email was shipped in the client-side admin.js bundle.
- [ ] Stored XSS in the public proposal-accept page title — A proposal titled with </title><script>… executed when a client opened the public /accept link
- [ ] Invoice builder wipes line items on every repaint — 'Add a line' did nothing and invoices could not be saved.
- [ ] Opening a picker modal from inside a form wiped the parent form — Opening the currency picker from inside New-lead / Settings / Invoice destroyed the parent mod…
...
- [ ] Business tier badge rendered washed-out and see-through — The BUSINESS badge next to the name showed as a pale, blurry, semi-transparent blob with unrea…
...
- [ ] e-business-card preview collapsed to a 2px sliver — The e-card live preview rendered as a 2px strip with its contents out of flow
...
- [ ] Notification panel could not be closed with its X — Clicking X on the notifications panel set hidden=true but the panel stayed on screen
- [ ] Sidebar collapse toggle sheared off by the sticky sidebar's overflow — The circular collapse chevron rendered clipped/half-cut at the sidebar's right edge
- [ ] Sidebar stopped scrolling partway down long pages — On pages taller than one screen the left sidebar cut off (couldn't reach items below 'Calls')
...
- [ ] AI outage: hardcoded Gemini model retired by the provider — All AI features returned errors; /api/ai gave 'gemini-2.0-flash is no longer available' (404)
...
- [ ] Magic-ring foundation stitches collapsed to one cell — Round 1 of nearly every amigurumi ('6 sc in magic ring') counted as a single stitch, undercoun…
...
- [ ] No brute-force lockout on 5-digit passcode sign-in/recovery — A 5-digit passcode could be guessed with unlimited attempts on sign-in and forgot-passcode
- [ ] Upsert ON CONFLICT omitted meta, dropping an edited prompt's asks ledger on sync — Editing a prompt lost its asks/convo/estimated data after cloud sync
...
- [ ] Landing-demo 'Sam' data bled into newly created accounts — Exploring the landing-page demo, then signing up, produced an account pre-filled with demo pro…
...
- [ ] Plans feature row wrapped its text below the tick — The long 'Automatic capture' feature line wrapped underneath its checkmark instead of beside it
...
- [ ] Voice-search mic threw because voiceSearch() was never defined — Tapping the voice-search mic did nothing and threw a ReferenceError
- [ ] Service worker cached a 404 for a later-added file — A vendor file added after first cache stayed permanently broken.
- [ ] Book-cover auto-fetch hung on a CORS blob fetch — Fetching a cover from Open Library hung/failed, so covers never appeared after adding a book
- [ ] Global search returned zero results after new sections were added — Search returned 0 hits while the app claimed you could 'find anything you've ever written'
- [ ] Monochrome theme hue not updating — Changing the mono hue had no effect.
- [ ] OCR web worker used relative paths that don't resolve in worker scope — Tesseract 'Magic Fill' pipeline failed to load its worker/core; status stalled and OCR never r…
...
- [ ] Bookshelf niche rendered dark in light themes because data-dark=0 still matched [data-dark] — In pastel/light themes the bookshelf back-panel niche rendered with the dark-mode material
...
## Budget LevelUp (15)
- [ ] Billable proxy endpoints unthrottled (denial-of-wallet) — Anyone could call /api/translate and /api/vision in a loop and run up billable third-party com…
...
- [ ] Debt payoff projection ignored APR — Debt-free date and total-interest figures were wrong — too optimistic.
...
## Aprizely (7)
- [ ] javascript:/data: URL scheme click-XSS via stored and AI-sourced links — A live_url/repo_url/pitch_url — or an AI-sourced competitor URL — set to javascript:… became a…
- [ ] One malformed report blanked the entire Reports list — The whole Reports view rendered blank when a single report had a string (not array) tasks field
- [ ] Break-even card said 'covers 100%' when revenue was 152% of costs — Finance dashboard displayed 'covers 100%' even though MRR was 152% of monthly costs
- [ ] Activity timeline dumped the entire status_note field — status_note edits printed the whole note into the activity timeline, cluttering it
...
- [ ] Negative currency rendered as $-59.17 instead of minus-$59.17 — Negative money values showed the minus sign after the dollar sign
...
- [ ] Wedding voice dictation dumped every spoken item into Guests — Speaking a mixed rundown (budget, vendor, song, gift, to-do) routed everything into the Guests…
...
## Hallalu Bookings (3)
- [ ] Concurrent bookings claim the same slot (double-booking race) — Two visitors open the same booking page, both see a slot as free, and both submit within secon…
- [ ] Availability rendered in owner's timezone, not the visitor's — Slots show the business's local hours to every visitor regardless of where they are, so an out…
...
## Breadcrumb Admin (1)
- [ ] Admin surface kept off the user-facing app — Putting admin oversight (which reads the app D1) inside the main app would widen its attack su…

## Breadcrumb Plan (1)
- [ ] Business plan and /deck must not leak on any direct path — A passcode-gated business plan plus /deck could leak via direct asset paths (plan.html.txt, sr…

## Bug Ledger Pitch (1)
- [ ] Deck chip truncated the name to M... — A slide chip clipped the label (e.g. M...) instead of showing the full name.

## Coco Modules (1)
- [ ] Live board cards blinked every poll — Activity timelines/boards re-appended cards each poll, replaying the CSS entrance animation so…

## Currency Picker (1)
- [ ] Currency picker logic duplicated per app — The 153-currency parse-search picker was re-implemented in each app, duplicating a whole class…

## Happy Travel (1)
- [ ] Recovery keys generated with Math.random (brute-forceable) — Cloud-data recovery keys came from Math.random (~15.5M combos); the key IS the password to a u…

## Kairos (1)
- [ ] Unsupportable 47% more productive landing claim — The landing asserted 47% more productive and other stats with no citable source.

## Kindly (1)
- [ ] Opaque gate never left, so Take-me-in did nothing — The sign-in gate stayed opaque over the booted app and the Take-me-in button appeared dead.
