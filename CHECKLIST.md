# BUG REGRESSION CHECKLIST — 337 known bugs across 18 apps
...
## cross-cutting (16)
- [ ] Excessive agency for AI features — An AI agent is given more tools, permissions or autonomy than its task needs, and can act with…
- [ ] Improper LLM output handling — Model output rendered or executed without validation, enabling XSS, SSRF, path traversal or co…
- [ ] Insecure or deprecated cryptography — Weak algorithms (MD5/SHA1 for passwords, ECB mode, hard-coded IV/salt, low KDF iterations).
- [ ] Log injection / unsanitized logging — User input written to logs unescaped (CWE-117), enabling forged log entries or downstream inje…
- [ ] Missing authorization / IDOR (object-level access) — Endpoints check authentication but not that the caller owns the specific record (broken object…
- [ ] Missing input sanitization / validation — The single most common flaw in AI-generated code — inputs used unsanitized in queries, HTML, f…
- [ ] No rate limiting on expensive / AI endpoints — Unauthenticated or unthrottled endpoints that call paid AI / browser-rendering / email can be…
- [ ] Prompt injection (direct and indirect) — Untrusted input (user text or content fetched from a page/doc/email) overrides the model's ins…
- [ ] Secrets committed / exposed to the client — API keys, tokens or private keys in source, client bundles or the repo (400+ found across vibe…
- [ ] SQL / NoSQL injection from string-built queries — Queries assembled by concatenating user input instead of parameterized.
- [ ] XSS from unescaped output rendered to HTML — User/model data written into the DOM without encoding — ~86% of AI-generated samples fail XSS…
- [ ] Happy-path assumption — no null/undefined guards — AI code assumes API/optional data is always present and dereferences it, crashing on the empty…
- [ ] Floating promises / unhandled async errors — Promises not awaited or lacking catch, so failures are silent and ordering is wrong.
- [ ] Hard-coded environment-specific values — localhost URLs, absolute paths, ports or keys baked in that break outside the author's machine.
- [ ] No error handling on network calls — fetch/HTTP calls with no try/catch, timeout or retry, so a blip becomes a broken UI.
- [ ] Silent catch swallowing errors — catch blocks that do nothing, hiding real failures and making bugs invisible.
...
- [ ] Automated git commit used `git add -A`, sweeping a co-editing session's work — The hourly harvester's auto-commit ran mid-work and its `git add -A` staged an in-progress ses…
